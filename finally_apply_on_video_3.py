...
def draw_lines(rgba_frame, lines):
    """Draw bounding boxes around lines of text."""
    if len(lines) >= 2:
        # Sort lines based on total width
        lines.sort(key=lambda x: sum(box[2] for box in x), reverse=True)
        # Draw bounding boxes for the two longest lines
        for line in lines[:2]:
